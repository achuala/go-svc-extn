...
func TestArgonHasher(t *testing.T) {
	...
	for _, pwLength := range []int{
		...
		t.Run(fmt.Sprintf("length=%dchars", pwLength), func(t *testing.T) {
			t.Parallel()
			pw := mkpw(t, pwLength)
			conf := hash.Argon2Configuration{Parallelism: 1, Memory: 128 * bytesize.MB, Iterations: 2, SaltLength: 16, KeyLength: 16}
			h := hash.NewHasherArgon2(&conf)
...
func TestSecretEnc(t *testing.T) {
	conf := hash.Argon2Configuration{Parallelism: 1, Memory: 128 * bytesize.MB, Iterations: 2, SaltLength: 16, KeyLength: 16}
	pw := []byte("pass@1234")
	h := hash.NewHasherArgon2(&conf)
	hs, err := h.Generate(context.Background(), pw)
...
func TestBcryptHasherGeneratesErrorWhenPasswordIsLong(t *testing.T) {
	t.Parallel()
	conf := hash.BcryptConfiguration{Cost: 10}
	hasher := hash.NewHasherBcrypt(&conf)
